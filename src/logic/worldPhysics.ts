...
export interface TrustRank {
    id: 'VETERAN' | 'REGULAR' | 'BEGINNER';
    label: string; // Placeholder for backward compatibility
    icon: string;
    color: string;
    bg: string;
    isVerified: boolean;
...
export const getTrustRank = (profile: UserProfile | null, snapshotScore: number = 0): TrustRank => {
    if (!profile) {
        ...
        if (snapshotScore >= 10) return { id: 'VETERAN', label: '熟練', icon: '🏆', color: 'text-amber-500', bg: 'bg-amber-100', isVerified: false };
        if (snapshotScore >= 3) return { id: 'REGULAR', label: '馴染み', icon: '★', color: 'text-blue-500', bg: 'bg-blue-100', isVerified: false };
        return { id: 'BEGINNER', label: '新顔', icon: '🔰', color: 'text-green-500', bg: 'bg-green-100', isVerified: false };
    }
    ...
    if (score >= 10 && isVerified && hasBio && hasAvatar) {
        return { id: 'VETERAN', label: '熟練', icon: '🏆', color: 'text-amber-500', bg: 'bg-amber-100', isVerified };
    }
    
    // Regular: Score 3+ & (Bio OR Avatar)
    if (score >= 3 && (hasBio || hasAvatar)) {
        return { id: 'REGULAR', label: '馴染み', icon: '★', color: 'text-blue-500', bg: 'bg-blue-100', isVerified };
    }
    
    // Beginner (Default)
    return { id: 'BEGINNER', label: '新顔', icon: '🔰', color: 'text-green-500', bg: 'bg-green-100', isVerified };
};
