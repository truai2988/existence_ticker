...
export const HomeView: React.FC<{ onOpenFlow: () => void; onOpenRequest: () => void; }> = ({ onOpenFlow, onOpenRequest }) => {
        ...
        <div className="relative w-[90%] max-w-[360px] aspect-square z-10">
          <div className="absolute inset-0 rounded-full shadow-2xl shadow-slate-200/50 border-4 border-white overflow-hidden bg-white">
            <svg viewBox="0 0 100 100" className="w-full h-full shape-rendering-geometricPrecision">
              <defs>
                <filter id="dividerGlow"><feGaussianBlur stdDeviation="1"/><feMerge><feMergeNode/><feMergeNode in="SourceGraphic"/></feMerge></filter>
                {/* HELP (Yellow) */}
                <linearGradient id="helpGrad" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#FEF3C7"/><stop offset="100%" stopColor="#FDE68A"/></linearGradient>
                {/* WISH (Blue) */}
                <linearGradient id="wishGrad" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#BFDBFE"/><stop offset="100%" stopColor="#DBEAFE"/></linearGradient>
                <linearGradient id="porcelain" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#F8FAFC"/><stop offset="100%" stopColor="#E2E8F0"/></linearGradient>
              </defs>
              ...
              <g>
                  ...
                  <path
                     ...
                     fill={(isRitualReady || isEmpty) ? "url(#porcelain)" : "url(#helpGrad)"}
                     className="transition-all duration-1000"
                  />
                  {/* Bottom Lobe (WISH - Blue) */}
                  <path
                     d="M 50 0 A 25 25 0 0 1 50 50 A 25 25 0 0 0 50 100 A 50 50 0 0 0 50 0 Z"
                     fill={(isRitualReady || isEmpty) ? "url(#porcelain)" : "url(#wishGrad)"}
                     className="transition-all duration-1000"
                  />
                  {/* Vertical S Divider */}
                  <path 
            ...
            {!isRitualReady && (
             <>
              <motion.button
                key="btn-help"
                onClick={onOpenFlow}
                className="absolute top-[25%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 outline-none group"
              ...
              >
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold tracking-[0.2em] opacity-90 leading-none">HELP</span>
              ...
              <motion.button
                ...
                className="absolute top-[75%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 outline-none group"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              >
                <motion.div 
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold tracking-[0.2em] opacity-90 mt-0.5 leading-none">WISH</span>
