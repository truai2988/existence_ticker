        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[70vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {applicants.length === 0 ? (
                <div className="py-8 text-center text-slate-600 text-base">{MESSAGES.WISH_CARD.MODAL_NO_APPLICANTS}</div>
              ) : (
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-100 text-center">
              <p className="text-sm text-slate-600">{MESSAGES.WISH_CARD.MODAL_SELECT_ONE}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl flex flex-col items-center justify-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-base text-slate-700 mb-6 text-center leading-relaxed font-sans">
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <button
                ...
                disabled={isLoading}
                className="px-4 py-2 text-base font-bold text-slate-600 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors font-sans w-full md:w-auto mt-2 md:mt-0"
              >
      ...
      {approvalTarget && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl flex flex-col items-center justify-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600 mb-6 text-center">{MESSAGES.WISH_CARD.MODAL_MSG_HINT}</p>
            <textarea
              value={contactNote}
