...
import { AlertTriangle, Clock, Handshake, Loader2, Archive } from "lucide-react";
import { WishCardState, WishCardHandlers } from "../types";
import { useWishActions } from "../../../hooks/useWishActions";
import { useToast } from "../../../contexts/ToastContext";

export const CardFooter: React.FC<{ state: WishCardState; handlers: WishCardHandlers }> = ({ state, handlers }) => {
  const {
    wish, isExpired, isMyWish, isReadOnly, isLoading, showApplicants, hasApplied, displayValue, currentUserId
  } = state;

  const {
    setShowApplicants, handleApply, handleCancel, handleCleanup, formatDate, setIsLoading
  } = handlers;
      ...
      <div className="flex flex-col gap-1 items-start">
        <div className="">
          {wish.status === "in_progress" && (
            <span className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 whitespace-nowrap shrink-0">進行中</span>
          )}
          ...
          {wish.status === "review_pending" && (
            <span className="text-xs font-bold text-purple-600 bg-purple-50 px-3 py-1 rounded-full border border-purple-100 animate-pulse whitespace-nowrap shrink-0">確認待ち</span>
          )}
          {(wish.status === "fulfilled" || wish.status === "completed") && (
            <span className="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full border border-green-100 whitespace-nowrap shrink-0">感謝済み</span>
          )}
          {wish.status === "expired" && (
            <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full border border-slate-200 whitespace-nowrap shrink-0">整理済み（期限切れ）</span>
          )}
          {wish.status === "open" &&
            (isExpired ? (
              ...
              <span className="text-xs font-bold text-slate-500 bg-slate-50 px-3 py-1 rounded-full border border-slate-200 whitespace-nowrap shrink-0">募集中</span>
            ))}
        </div>
        ...
        {isMyWish && (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 ml-1">
            ...
            {wish.isAnonymous && <span className="text-xs font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded-full border border-red-100 uppercase tracking-tight">匿名</span>}
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end">
          ...
          <>
            ...
            {!isExpired && !isReadOnly && (wish.status === "review_pending" || wish.status === "in_progress") && (
              <div className="flex flex-col items-end gap-2">
                <p className="text-xs font-bold text-slate-700">実費（材料費など）の清算が済んでいることを確認し、感謝の Lm を贈ります。</p>
                <button
                  onClick={() => {
                    if (
                      confirm(
        ...
        {!isMyWish && !isExpired && !isReadOnly && (
          <>
              ...
              <div>
                {hasApplied ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200 hover:bg-amber-500 hover:text-white hover:border-amber-500 text-sm font-bold transition-all shadow-sm hover:shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Handshake className="w-4 h-4" />}
                    <span>応える</span>
                  </button>
                )}
              </div>
            ...
            {(wish.status === "in_progress" || wish.status === "review_pending") && !isReadOnly && wish.helper_id === currentUserId && (
