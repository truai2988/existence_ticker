...
export const JournalView: React.FC<JournalViewProps> = ({ onTabChange, onOpenGuide }) => {
  const { user } = useAuth();
            ...
            <div className="max-w-2xl mx-auto px-6 py-4 md:py-6 flex items-start justify-between">
                ...
                {onTabChange && (
                    <div className="flex h-12 items-end gap-2 shrink-0">
                        <HeaderNavigation currentTab="history" onTabChange={(tab: AppViewMode) => onTabChange(tab)} onOpenGuide={onOpenGuide} />
                    </div>
                )}
            </div>
        </div>
